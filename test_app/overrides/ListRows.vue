<template>
    ...
    <div v-for="group in groupedRows" :key="group.group">
      ...
      <ListGroupRows :group="group" id="list-rows" class="!mt-0">
        <ListRow
          ...
          :row="row"
          :class="['truncate text-base row', getRowClass(row)]"
        >
  ...
  <ListRows class="mx-3 sm:mx-5" v-else id="list-rows">
    <ListRow
      ...
      v-slot="{ idx, column, item }"
      :row="row"
      :class="['truncate text-base', getRowClass(row)]"
    >
      <slot v-bind="{ idx, column, item, row }" />
    </ListRow>
  </ListRows>
...
// Inject the parent component's options if available
const parentOptions = inject('listViewOptions', null);
...
const getRowClass = (row) => {
  ...
  if (parentOptions?.rowClass) {
    return parentOptions.rowClass(row);
  }
  return '';
};
...
<style scoped>
/* Ensure row backgrounds fill completely */
:deep(.list-row) {
