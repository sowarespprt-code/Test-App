...
import { Breadcrumbs, call, Dropdown, toast, createResource } from "frappe-ui";
import {
  computed,
  ComputedRef,
  h,
...
//function handleAssigneeEvent(data: any) {
  //if (data.name === ticket.value.name) {
    //console.log("🔥 Assignee updated via realtime event");
    
    // Reload both ticket and assignees
    //Promise.all([
      //ticket.value.reload(),
...
//onMounted(() => {
  //if (window.frappe?.realtime) {
    //window.frappe.realtime.on("helpdesk:ticket-assignee-update", handleAssigneeEvent);
  //}
//});

//onUnmounted(() => {
  //if (window.frappe?.realtime) {
    //window.frappe.realtime.off("helpdesk:ticket-assignee-update", handleAssigneeEvent);
  //}
//});
