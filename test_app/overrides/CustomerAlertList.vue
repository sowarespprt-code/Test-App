<template>
  <div class="flex flex-col h-full">
    ...
    <!-- Alert List WITH SPINNER - FIXED STRUCTURE -->
    <div class="flex-1 overflow-auto">
      <!-- LOADING SPINNER -->
      <div v-if="isLoading" class="flex items-center justify-center h-64">
        <div class="flex flex-col items-center gap-3">
          <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
          <div class="text-sm text-gray-600">Loading customer alerts...</div>
        </div>
      </div>

      <!-- NO ALERTS FOUND - COMPLETE REWRITE -->
      <template v-else>
          ...
          <table class="w-full divide-y">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="alert in filteredAlerts" :key="alert.name" class="hover:bg-gray-50 cursor-pointer transition-colors" @click="openAlertDetail(alert)">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm font-medium text-gray-900">{{ alert.customer_display_name || alert.customer_name || 'N/A' }}</div>
                </td>
                <td class="px-6 py-4">
        ...
        <div v-else-if="!alerts.length" class="flex h-full flex-col items-center justify-center gap-4 p-8">
          <div class="text-lg text-gray-500">No customer alerts found</div>
        ...
        <div v-else class="flex h-full flex-col items-center justify-center gap-4 p-8">
          <div class="text-lg text-gray-500">No alerts found for "{{ searchQuery }}"</div>
...
async function fetchAlerts() {
  ...
  try {
    ...
    const uniqueCustomerIds = [...new Set(data.map((alert: any) => alert.customer_name).filter(Boolean))];
    ...
    if (uniqueCustomerIds.length > 0) {
      const customers = await call("frappe.client.get_list", {
        doctype: "HD Customer",
        filters: [["name", "in", uniqueCustomerIds]],
        fields: ["name", "customer_name"],
      });
      
      
      customers.forEach((customer: any) => {
        customerMap[customer.name] = customer.customer_name;
      });
    }
    
    
    alerts.value = data.map((alert: any) => ({
      ...alert,
      customer_display_name: customerMap[alert.customer_name] || alert.customer_name || "N/A"
    }));
...
const filteredAlerts = computed(() => {
  ...
  return alerts.value.filter((item: any) => 
    item.customer_display_name?.toLowerCase().includes(query) ||
